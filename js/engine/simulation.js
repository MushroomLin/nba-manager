...
const SimEngine = (() => {
    ...
    //   系数校准依据：2018 骑士（詹姆斯 offOf~95 + 角色~72）真实 ORTG
    //   联盟第 5（+1.7 vs 平均）。
    //   修复 v21：0.40→0.55。原 0.40 导致"3 个 87 进攻球星 + 62 配角"的队
    //   进攻效率仅 78.8，低于全队 77 的均衡队（实测胜率 44%）——球星
    //   集中度严重不足。真实 NBA 球星使用率 35%+（关键时刻完全接管），
    //   太阳三巨头（布克+KD+比尔，配角平庸）进攻仍是联盟第 8。
    //   0.55 时该阵容 off≈82（+3.2），胜率回归 ~57%，符合真实三巨头球队
    //   （2018 骑士/2024 太阳 49-50 胜）量级。
    //   防守星光加成见下方 defWithStarPower（v15/v21）。
    const offOf = p => (p.ins + p.sh + p.pa) / 3;
    const STAR_POWER_COEF = 0.55;
    ...
    //   修复：防守效率 = 全队分钟加权均值 + (前三防守点加权均值 - 全队均值) × 0.45
    //   修复 v21：0.30→0.45。原 0.30 时"三球星(de 75-80) + 弱配角(de 58-66)"
    //   的队防守 73，被全 77 均衡队打爆（用户："明星很多打不过平民队"）。
    //   真实 NBA 防守可部分隐藏弱者（换防/协防/藏人），且 de 高的球星
    //   主防对方箭头（2018 勇士库里防守一般但杜兰特/格林/汤普森锚定
    //   体系，DRTG 联盟第 11 非倒数）。
    //   系数 0.45 仍低于进攻 0.55：进攻球权可完全集中，防守只能部分隐藏
    //   （2018 骑士角色防守差，DRTG 仍联盟第 29 —— 极端防守漏洞队不会
    //   因球星加成变防守强队）。
    //   均衡队 top3≈avg 加成≈0，联盟整体格局不受影响。
    const DEF_STAR_COEF = 0.45;
